 ...
 *
 * dm-default-key: whole-device metadata encryption for f2fs/ext4.  Adapted to
 * the Rockchip develop-6.1 base blk-crypto API (standard keys only; this SoC
 * has no hardware-wrapped-key inline crypto engine, so all encryption goes
 * through blk-crypto-fallback).
 */
...
static int default_key_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	if (raw_key_size > 2 * BLK_CRYPTO_MAX_KEY_SIZE ||
	    raw_key_size % 2) {
	...
	if (raw_key_size != cipher->key_size) {
		ti->error = "Incorrect keysize for cipher";
		err = -EINVAL;
		goto bad;
	}
